...
import { findService, formatAmount, standardHourlyRate } from '@/content/services'
...
const faqs = [
  ...
  { q: 'Wat als ik meer uren nodig heb dan mijn pakket?', a: `Extra uren boven de pakketlimiet kosten $${standardHourlyRate} per uur. We melden het altijd vooraf zodra een aanvraag buiten uw resterende uren valt, zodat u zelf kiest of het deze maand of volgende maand wordt.` },
...
function monthlyPrice(id: string) {
  const service = findService(id)
  if (!service || !('amount' in service.price)) throw new Error(`Unknown UX Kukru service: ${id}`)
  return `$${formatAmount(service.price.amount)}`
}

const pricing = [
  { name: 'Starter Support', price: monthlyPrice('kukru-starter'), desc: '10 uur per maand voor kleine, doorlopende taken', features: ['10 uur per maand', 'Kleine updates + bug fixes', 'Content wijzigingen', 'Technisch advies', 'Reactie binnen 48 uur'], highlight: false },
  { name: 'Business Support', price: monthlyPrice('kukru-business'), desc: '20 uur per maand voor doorontwikkeling', features: ['20 uur per maand', 'Nieuwe functies bouwen', 'Design updates + integraties', 'Strategisch advies', 'Reactie binnen 24 uur'], highlight: true },
  { name: 'Partner Support', price: monthlyPrice('kukru-partner'), desc: '40 uur per maand met vaste bezetting', features: ['40 uur per maand', 'Vast aanspreekpunt', 'Voorrang bij complexe projecten', 'Samenwerking met uw team', 'Reactie binnen 12 uur'], highlight: false },
]
      ...
      <section id="pricing" className="bg-gradient-to-b from-violet-50 to-white py-16">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-500 max-w-lg mx-auto">Dezelfde prijzen als op onze dienstenpagina. Extra uren ${standardHourlyRate} per uur, minimaal drie maanden.</p>
          </div>
